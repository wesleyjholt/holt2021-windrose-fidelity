...
output_layout_directory = "final-layouts/9turb-circle-450radius/" * lpad(ndirs,3,"0") * "dirs/"
...
for i = 1:length(wec_values)
    ...
    options["Summary file"] = "opt-files/9turb-circle-450radius/" * lpad(ndirs,3,"0") * "dirs/" * "snopt-" * lpad(layout_number,3,"0") * "-wec" * rpad(round(wec_values[i],digits=1),3,"0") * "-summary.out"
    options["Print file"] = "opt-files/9turb-circle-450radius/" * lpad(ndirs,3,"0") * "dirs/" * "snopt-" * lpad(layout_number,3,"0") * "-wec" * rpad(round(wec_values[i],digits=1),3,"0") * "-print.out"
    xopt_all[:,i+1], fopt, info = snopt(obj_func, x, lb, ub, options)
end

localtimodel = ff.LocalTIModelMaxTI()
model_set = ff.WindFarmModelSet(wakedeficitmodel, wakedeflectionmodel, wakecombinationmodel, localtimodel)
options["Summary file"] = "opt-files/9turb-circle-450radius/" * lpad(ndirs,3,"0") * "dirs/" * "snopt-" * lpad(layout_number,3,"0") * "-wec" * rpad(round(wec_values[end],digits=1),3,"0") * "-TI-summary.out"
options["Print file"] = "opt-files/9turb-circle-450radius/" * lpad(ndirs,3,"0") * "dirs/" * "snopt-" * lpad(layout_number,3,"0") * "-wec" * rpad(round(wec_values[end],digits=1),3,"0") * "-TI-print.out"

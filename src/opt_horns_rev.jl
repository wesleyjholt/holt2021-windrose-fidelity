#=
AUTHOR: Wesley Holt
PURPOSE: Optimizes the turbine coordinates for a specified wind farm.
=#
...
########################################################################################################
# INPUT ARGUMENTS: number of directions, number of speeds, layout_number, wake model
########################################################################################################
ndirs = lpad(ARGS[1], 3, "0")
nspeeds = lpad(ARGS[2], 2, "0")
farm_name = "horns-rev"
layout_number = lpad(ARGS[3], 3, "0")
wake_model = ARGS[4]
...
layout_directory = "initial-layouts/"* farm_name * "/"
layout_filename = "initial-layout-" * layout_number * ".txt"
windrose_directory = "windrose-files/" * farm_name * "/"
windrose_filename = "horns-rev-windrose-" * ndirs * "dirs-" * nspeeds * ".txt"
final_layout_directory = "final-layouts/"* farm_name * "/" * wake_model * "/snopt-wec/" * ndirs * "dirs/" * nspeeds * "speeds.txt"
final_layout_filename = "final-layout-" * layout_number * ".yaml"
final_wec_layouts = "final-layout-" * layout_number * "-history.txt"


########################################################################################################
# SET UP FUNCTIONS FOR THE OPTIMIZER
########################################################################################################
...
if wake_model == "gaussian"
    include(data_directory * "model-sets/model_set_9turb_round_farm.jl")
elseif wake_model == "jensen"
    include(data_directory * "model-sets/model_set_9turb_round_farm.jl")
    wakedeficitmodel = ff.JensenCosine()
    model_set = ff.WindFarmModelSet(wakedeficitmodel, wakedeflectionmodel, wakecombinationmodel, localtimodel)
else
    error("Please specify valid wake model.")
end
...
for i = 1:length(wec_values)
    ...
    options["Summary file"] = "opt/9turb/circle/" * boundary_radius_string * "r/" * wake_model * "/snopt-wec/" * ndirs * "dirs/" * layout_number * "-wec" * wec_string * "-summary.out"
    options["Print file"] = "opt/9turb/circle/" * boundary_radius_string * "r/" * wake_model * "/snopt-wec/" * ndirs * "dirs/" * layout_number * "-wec" * wec_string * "-print.out"
...
# set Snopt options
cd(results_directory)
options["Summary file"] = "opt/9turb/circle/" * boundary_radius_string * "r/" * wake_model * "/snopt-wec/" * ndirs * "dirs/" * layout_number * "-wec" * wec_string * "TIsummary.out"
options["Print file"] = "opt/9turb/circle/" * boundary_radius_string * "r/" * wake_model * "/snopt-wec/" * ndirs * "dirs/" * layout_number * "-wec" * wec_string * "TIprint.out"
options["Major optimality tolerance"] = opt_tol[end]

# call Snopt (last optimization with TI and WEC=1)
...
ff.write_turb_loc_YAML(results_directory * final_layout_directory * final_layout_filename, turbine_x, turbine_y; 
    ...
    turbinefile="../data/input-files/NREL5MWCPCT.txt", locunits="m", wakemodelused=wake_model, windresourcefile="../data/windrose-files/nantucket-windrose-ave-speeds-" * lpad(ndirs,3,"0") * "dirs.txt", aeptotal=final_obj*1e-6/params.obj_scale, 
    aepdirs=ndirs, aepunits="MWh", baseyaml="../data/initial-layouts/default_turbine_layout.yaml")

# save WEC layout history to txt file
open(results_directory * final_layout_directory * final_wec_layouts, "w") do io

...
# this file is designed to be run only when called by opt_9turb.jl
...
# get turbine data
turbinepowerdata = readdlm(data_directory * "input-files/vestas_v80_2MW_power_curve.txt", skipstart=1)
turbinethrustdata = readdlm(data_directory * "input-files/vestas_v80_2MW_thrust_coefficient_curve.txt", skipstart=1)
velpoints_power = turbinepowerdata[:,1]
powerpoints = turbinepowerdata[:,2]
velpoints_ct = turbinethrustdata[:,1]
...
power_model = ff.PowerModelPowerPoints(velpoints_power, powerpoints)
...
ct_model = ff.ThrustModelCtPoints(velpoints_ct, ctpoints)
